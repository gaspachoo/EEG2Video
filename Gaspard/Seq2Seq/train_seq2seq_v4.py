...
def train_seq2seq(args):
    ...
    # Initialisation Weights & Biases si nécessaire
    if args.use_wandb:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        model.train()
        train_loss = 0.0
        for batch_idx, (src, tgt) in enumerate(tqdm(train_loader, desc=f"Train Epoch {epoch}")):
            ...
            # Logging partiel pour ce batch sur wandb
            if args.use_wandb:
                wandb.log({
                    'batch_loss': loss.item(),
                    'batch_diff_mean': diff_mean,
                    'batch_diff_std': diff_std,
        ...
        print(f"Epoch {epoch:03d}  train_loss={avg_train:.4f}  val_loss={avg_val:.4f}  val_diff_mean={avg_diff_mean_val:.6f}  val_diff_std={avg_diff_std_val:.6f}  lr={optimizer.param_groups[0]['lr']:.6e}")

        # Sauvegarde du meilleur modèle
        if avg_val < best_val:
            best_val = avg_val
            os.makedirs(args.save_path, exist_ok=True)
            ckpt = os.path.join(args.save_path, 'seq2seq_v3_best.pth')
    ...
    model.eval()
    test_loss = 0.0
    test_diff_mean_sum = 0.0
    test_diff_std_sum = 0.0
    with torch.no_grad():
        for src, tgt in tqdm(test_loader, desc="Test"):
            ...
            diff = out - tgt
            test_diff_mean_sum += diff.mean().item()
            test_diff_std_sum += diff.std().item()
    avg_test = test_loss / len(test_loader)
    avg_test_diff_mean = test_diff_mean_sum / len(test_loader)
    avg_test_diff_std = test_diff_std_sum / len(test_loader)
    print(f"Test final  ► test_loss={avg_test:.4f}  test_diff_mean={avg_test_diff_mean:.6f}  test_diff_std={avg_test_diff_std:.6f}")

    if args.use_wandb:
        wandb.log({'test_loss': avg_test,
                   'test_diff_mean': avg_test_diff_mean,
                   'test_diff_std': avg_test_diff_std})
        wandb.finish()
