...
class TuneAVideoTrainer:
    ...
    def _step(self, z0, et):
        ...
        z0 = z0.permute(0,2,1,3,4).contiguous()
        et = et.to(self.device).unsqueeze(1)
        et = self.proj_eeg(et).contiguous()

        noise = torch.randn_like(z0)
        
        # uniform in [0, num_train_timesteps-1]
        timesteps = torch.randint(0,self.scheduler.config.num_train_timesteps,(z0.size(0),),device=self.device).to(torch.long)
        z_t = self.scheduler.add_noise(z0, noise, timesteps).contiguous()
