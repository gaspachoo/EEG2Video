...
def parse_args():
    ...
    parser.add_argument('--stats_path',    type=str, default="./EEG2Video/checkpoints/seq2seq",
                        help="Où sauvegarder mean_z et std_z (défaut: save_path)")
    parser.add_argument('--ckpt_name',     type=str,   default='seq2seq_v3_color.pth',help='Name of the saved checkpoint file')
    ...
    # Train/val/test ratios (mandatory)
    ...
    # test_ratio will be computed as 1 - train_ratio - val_ratio
...
def load_and_prepare_data(args):
    # Load EEG embeddings, supports shape (7,40,5,7,512) or (9800,512)
    ...
    # Load and concatenate video latents from all blocks
    z0_list = []
    num_blocks = 7
    ...
    # Compute mean/std and normalize if requested
    if args.normalize:
...
def train_seq2seq(args):
    ...
    if args.use_wandb:
        wandb.init(project="eeg2video-Seq2Seqv3", config=vars(args))
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for src, tgt in tqdm(train_loader, desc=f"Train Epoch {epoch}"):
            ...
            preds = []
            tgt_pred = torch.zeros(tgt.size(0), 1, tgt.size(2), device=device)

            optimizer.zero_grad()
            for _ in range(tgt.size(1)):
                out = model(src, tgt_pred)
                preds.append(out[:, -1:])
                tgt_pred = torch.cat([tgt_pred, out[:, -1:]], dim=1)
    ...
    with torch.no_grad():
        for src, tgt in tqdm(test_loader, desc="Test"):
            ...
            preds = torch.cat(preds, dim=1)
            test_loss += criterion(preds, tgt).item()
    avg_test = test_loss / len(test_loader)
    print(f"Final test  ► test_loss={avg_test:.4f}")
