...
def parse_args():
    ...
    parser.add_argument(
        ...
        help='Directory with raw EEG segments generated by segment_sliding_window.py'
    )
    parser.add_argument(
        '--video_dir',
        type=str,
...
def load_eeg_data(raw_path: str):
    """Load EEG data from ``raw_path``."""
...
def build_dataset(raw_dir: str, video_dir: str):
    """Build dataset from raw EEG files and shared video latents."""
    ...
    for fname in sorted(os.listdir(raw_dir)):
        ...
        raw = load_eeg_data(raw_path)

        if raw.shape[0] != video_latents.shape[0]:
            raise ValueError(
                f'Mismatch between EEG samples ({raw.shape[0]}) and video latent length ({video_latents.shape[0]})'
            )

        raw_all.append(raw)
        video_all.append(video_latents)

    raw_concat = np.concatenate(raw_all, axis=0)
    video_concat = np.concatenate(video_all, axis=0)
    return Dataset(raw_concat, video_concat)
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for eeg, video in tqdm(train_loader, desc=f'Train {epoch}'):
            ...
            pad = torch.zeros(b, 1, c, h, w, device=device)
            tgt_in = torch.cat([pad, video[:, :-1]], dim=1)
            optim.zero_grad()
            _, out = model(eeg, tgt_in, teacher_forcing=True)
        ...
        with torch.no_grad():
            for eeg, video in val_loader:
                ...
                tgt_in = torch.cat([pad, video[:, :-1]], dim=1)
                _, out = model(eeg, tgt_in, teacher_forcing=True)
                val_loss += criterion(out, video).item()
    ...
    with torch.no_grad():
        for eeg, video in test_loader:
            ...
            tgt_in = torch.cat([pad, video[:, :-1]], dim=1)
            _, out = model(eeg, tgt_in, teacher_forcing=True)
            test_loss += criterion(out, video).item()
